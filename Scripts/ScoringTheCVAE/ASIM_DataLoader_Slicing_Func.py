...
def ASIM_DataLoader(dataDir, loadStatusUpdate,minval, maxval, p, sliceSize):
        ...
        for count, file in enumerate(fileListFlatten):
          ...
          #          Obs_ID.append(Observation_ID[0])
                    ...
                    while currentSlice < len(PHOT1_Data) - sliceSize:
                        ...
                        name.append(file) # Save frame number and file name, so we can find it's class later
                        frame.append(f)
                        f = f+1 # Add to frame numer
    ...
    class Dataset(torch.utils.data.Dataset):
      ...
      def __init__(self, P1D, P2D, P3D, Time, name, frame):
            'Initialization'
      ...
      def __getitem__(self, idx):
            ...
            return P1, P2, P3, Time, name, frame
    ...
    CollectedData = Dataset(NormalizedDataset(P1D,minval,maxval), NormalizedDataset(P2D,minval,maxval), NormalizedDataset(P3D,minval,maxval), Time, name, frame)
