...
def plot_autoencoder_stats(
    ...
    colors = iter(plt.get_cmap('Set1')(np.linspace(0, 1.0, len(classes)))) # RCJ Edited
    for c in classes:
        ax.scatter(*z[y.numpy() == c].T, c=next(colors), marker='o')
    #ax.scatter(*z, marker='o') # RCJ EDITTED
    ax.legend(classes)
...
#def plot_2d_latents(ax, qz, z): # RCJ Edited
    z = z.to('cpu')
    y = y.to('cpu') # RCJ Edited
    scale_factor = 2
    batch_size = z.shape[0]
    palette = sns.color_palette()
    colors = [palette[l] for l in y] # RCJ Edited
...
#def plot_latents(ax, z): # RCJ Edited
    z = z.to('cpu')
    palette = sns.color_palette()
    colors = [palette[l] for l in y] # RCJ Edited
...
def make_vae_plots(vae, x, y, outputs, training_data, validation_data, tmp_img="tmp_vae_out.png", figsize=(18, 18)): # RCJ Edited
